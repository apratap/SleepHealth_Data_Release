...
sharingInfo <- gdata::read.xls(synGet("syn21958546")$path) %>%
  dplyr::mutate(sharing = stringr::str_trim(sharing))
...
underage_participants <- unique(underage_participants$participantId)

#Remove test accounts
test_participants <- gdata::read.xls(synGet("syn21958537")$path )
test_participants <- unique(test_participants$participantId)
...
tmp_get_QAQC <- function(path){
  df <- fread(path, data.table = F, quote="", fill=T, header = T) %>%
    ...
    filter(! participantId %in% underage_participants ) %>%
    # Filter out test participants
    filter(! participantId %in% test_participants)
  ...
  data.frame(nUsers, nUsers_broadShare)
}
...
View(sh_hk_data_tobe_released %>% select(name, nUsers_broadShare, percent_broadShare, nDays))
...
df <- fread(synGet("syn21292879")$path, data.table = F)  %>% 
  ...
  #Filter out underage participants
  filter(! participantId %in% underage_participants) c
